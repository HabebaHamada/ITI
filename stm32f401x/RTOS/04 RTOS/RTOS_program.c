...
u8 RTOS_u8CreateTask(void(*Copy_TaskHandler)(void),u8 Copy_u8Priority, u16 Copy_u16Periodicity, u16 Copy_u16FirstDelay, u16 Copy_u16TaskTiming)
{
...
void Scheduler(void)
{
	...
	/* Loop on all Taks */
	for (Local_u8Counter =0; Local_u8Counter < MaxNumberOfTasks; Local_Counter++)
	{
		/* Is there a registered task ... ? */
		if (SystemTasks[Local_u8Counter].TaskHandler != 0)
		{	
			if (SystemTasks[Local_Counter].TaskState == Ready )
			{
				if (SystemTasks[Local_Counter].TaskTiming == 0)
				{
					/* Run the task */
					SystemsTasks[Local_u8Counter].TaskHandler();
					
					/*Task state is Running*/
				    SystemTasks[Local_Counter].TaskState = Running;
					
					/* Reload the periodicity */
					SystemTasks[Local_Counter].TaskTiming = Copy_u8Periodicity;
				}
			
				else 
				{
					SystemTasks[Local_Counter].TaskTiming--;
				}
			}
			else if(SystemTasks[Local_Counter].TaskState == Running )
			{
				if (SystemTasks[Local_Counter].TaskTiming == 0)
				{
				  SystemTasks[Local_Counter].TaskState = Ready;
			...
			else if(SystemTasks[Local_Counter].TaskState == Suspended )
			{
				if (SystemTasks[Local_Counter].SuspendTime == 0)
				{
				  SystemTasks[Local_Counter].TaskState = Ready;
				...
				{
					SystemTasks[Local_Counter].SuspendTime--;
				}
...
{
	STK_SetIntervalPeriodic(TICK_TIME,Scheduler);
}